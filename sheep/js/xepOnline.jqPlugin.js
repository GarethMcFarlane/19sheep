...
xepOnline.Formatter = {
	...
	getFormData: function(PrintCopy, Name, MimeType, FileName, Resolution) {
		...
		try
		{
			if(__deniz__debug__)window.alert("Name = " + Name + " FileName = " + FileName);
			blob = new Blob([data],{ type: xepOnline.Formatter.src_type.xml });
		}
		catch(e) 
		{
		
			if(__deniz__debug__)window.alert("getFormData Failed");
	...
	*/
	__format: function(ElementIDs, options) {
		...
		if(__deniz__debug__)window.alert("current_height = " + current_height);
		...
		if(options.render === 'none') {
			if(__deniz__debug__)window.alert("options.render === 'none' returning false");
			return false;
		}
		
		if(__deniz__debug__)window.alert("options.render = " + options.render);
		...
		if(options.render !== 'download') {
			try 
			{
				if(__deniz__debug__)window.alert("Calling getFormData for a printcopy");
				blob = xepOnline.Formatter.getFormData(printcopy, 'xml', options.mimeType, 'document.xml', options.resolution);
			} catch(e) 
			{
				if(__deniz__debug__)window.alert("render failed");
...
'<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" \
	...
	Format: function(ElementID, options) {
		...
		if(__deniz__debug__)window.alert("ElementID = " + ElementID + " options = " + options);
		
		if(jQuery.isArray(ElementID)) {
			if(__deniz__debug__)window.alert("jQuery.isArray(ElementID) = true");
			items = ElementID;
		} else {
			if(__deniz__debug__)window.alert("jQuery.isArray(ElementID) = false");
			items = [ ElementID ];
		}
